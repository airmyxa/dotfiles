...
var mockProvider *database.MockProvider

func TestMain(m *testing.M) {
	// Setup
	provider, err := database.NewMockProvider()
	if err != nil {
		panic(err)
	}
	...
	// Cleanup
	if err := mockProvider.Close(); err != nil {
		panic(err)
	}
...
func buildTestContainer() *dig.Container {
	...
	if err := container.Provide(func() *database.MockProvider { return mockProvider }); err != nil {
		return nil
	}
