...
var simpleTestProvider *database.SimpleTestProvider
...
func TestMain(m *testing.M) {
	...
	// Cleanup
	if err := simpleTestProvider.Close(); err != nil {
		panic(err)
	}
...
func buildTestContainer() *dig.Container {
	...
	// Register simple test database provider
	if err := container.Provide(func() *database.SimpleTestProvider { return simpleTestProvider }); err != nil {
		return nil
	}
	if err := container.Provide(func(p *database.SimpleTestProvider) *sqlx.DB { return p.GetDB() }); err != nil {
		return nil
	}

	// Register custom repository provider for tests
	if err := container.Provide(func(db *sqlx.DB) config.RepositoryInterface {
