...
func TestConfigVersioning(t *testing.T) {
	...
	_, err := service.Create(ctx, name, initialData)
	if err != nil {
		t.Fatalf("Failed to create initial config: %v", err)
	}

	// Test GetAtTime functionality by checking if we can get the current version
	...
	if version, ok := currentVersion.Data["version"].(float64); !ok || int(version) != 1 {
		t.Errorf("Expected version 1, got %v (%T)", currentVersion.Data["version"], currentVersion.Data["version"])
	}

	// Wait a full second to ensure the timestamp changes
	...
	updatedVersion, err := service.Get(ctx, name)
	if err != nil {
		t.Fatalf("Failed to get updated version: %v", err)
	}

	// Verify we got the updated version
	if version, ok := updatedVersion.Data["version"].(float64); !ok || int(version) != 2 {
		t.Errorf("Expected version 2, got %v (%T)", updatedVersion.Data["version"], updatedVersion.Data["version"])
	}
}
