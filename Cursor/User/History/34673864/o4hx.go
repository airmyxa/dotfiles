...
func TestMain(m *testing.M) {
	...
	provider, err := database.NewTestPostgresProvider([]string{"../../postgres/migrations/V1_init.sql"})
	if err != nil {
		panic(err)
	}
...
func setupTest(t *testing.T) (*Service, *Repository, context.Context) {
	db := testDB.(*database.TestPostgresProvider).DB
...
func TestConfigVersioning(t *testing.T) {
	...
	originalVersion, err := service.GetAtTime(ctx, name, initialTimestamp)
	if err != nil {
	...
	if version, ok := originalVersion.Data["version"].(float64); !ok || int(version) != 1 {
		t.Errorf("Expected version 1, got %v", originalVersion.Data["version"])
	}
	...
	if version, ok := currentVersion.Data["version"].(float64); !ok || int(version) != 2 {
		t.Errorf("Expected version 2, got %v", currentVersion.Data["version"])
	}
}
