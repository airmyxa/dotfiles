...
// getNowFunc returns the appropriate NOW function based on the database driver
func (r *Repository) getNowFunc() string {
...
func (r *Repository) Create(ctx context.Context, cfg *Config) error {
	nowFunc := r.getNowFunc()
	query := `INSERT INTO configs (name, data, updated_at) VALUES ($1, $2, ` + nowFunc + `)`
...
func (r *Repository) Update(ctx context.Context, cfg *Config) error {
	nowFunc := r.getNowFunc()
	query := `UPDATE configs SET data=$1, updated_at=` + nowFunc + ` WHERE name=$2`
